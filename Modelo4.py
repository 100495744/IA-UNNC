...
def generate(
        ...
        idx2word,
        word2idx,
        length=100,           # número de palabras a generar
        temp=0.8,             # temperatura
        top_k=50,             # top-k sampling
        device="cuda",
        recent_window=40      # ventana para penalizar repeticiones
):
    """
    Genera texto a partir de un modelo entrenado (palabra a palabra) usando Multihead Attention.
    Mejora: penalización proporcional a frecuencia + top-k + control adaptativo de <eos>.
    """
    import torch
    ...
    # Convertir start_words a lista de palabras
    ...
    for _ in range(length):
        # Últimos SEQ_LEN tokens como contexto
        x = torch.tensor([idxs[-SEQ_LEN:]], device=device)
        logits, hidden = model(x, hidden)  # [1, vocab_size]
        logits = logits.squeeze(0) / temp

        # Penalizar palabras recientes según frecuencia
        recent_window_words = idxs[-recent_window:]
        counts = Counter(recent_window_words)
        for word_idx, count in counts.items():
            logits[word_idx] -= count * 2.0  # más penalización si aparece más veces

        # Penalización de <eos> para no cortar demasiado pronto
